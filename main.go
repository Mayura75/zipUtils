...
func fixJarFile(inputPath, outputPath string) error {
	...
	for currentIndex := range reader.File {
		currentFile := reader.File[currentIndex]
		name := strings.TrimSuffix(currentFile.Name, "/") // Remove trailing slash if present (Python zipfile quirk)

		isClassFile := strings.HasSuffix(name, ".class")
		isDirectoryEntry := currentFile.Mode().IsDir()
		isZeroSizeDir := isDirectoryEntry && currentFile.UncompressedSize64 == 0
		...
		currentFile.Name = strings.TrimSuffix(currentFile.Name, "/") // Remove trailing slash if present (Python zipfile quirk)
